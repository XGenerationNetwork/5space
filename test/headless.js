...
function stageDeploy() {
  ...
  /* three.js is the only third-party code in the repository, and the reason
     it is committed rather than fetched is the offline guarantee above.  The
     loop over PAGES already proves the tag is relative and the file is there;
     what it cannot see is someone "tidying up" by swapping in a CDN URL, or
     the build turning out to be an ES module - which would not load from a
     file:// path, and file:// is a supported way to run this. */
  const vendored = path.join(root, 'vendor/three.min.js');
  ok(fs.existsSync(vendored), 'three.js is vendored, not fetched at runtime');
  if (fs.existsSync(vendored)) {
    const three = fs.readFileSync(vendored, 'utf8');
    ok(three.length > 100000, 'the vendored three.js is a real build');
    ok(/typeof exports/.test(three) && /typeof define/.test(three),
      'it is the UMD build, so it works from file:// without modules');
    ok(three.indexOf('deprecated') < 0,
      'and not a build that nags about deprecation on every page load');
    ok(/MIT/.test(three.slice(0, 400)), 'the licence header is intact');
  }

  const home = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
  ok(/<script src="vendor\/three\.min\.js">/.test(home),
    'the welcome page loads three.js from vendor/, not from a CDN');
...
function stageDocs() {
  ...
  /* The animated masthead is decoration, and the thing that matters about
     decoration is that it never becomes load-bearing.  Each of these is a way
     it could quietly stop being optional. */
  ok(/<div class="masthead"[^>]*aria-hidden="true"/.test(manual),
    'the masthead is hidden from assistive tech rather than described twice');
  ['!window.THREE', 'prefers-reduced-motion', 'IntersectionObserver',
   'visibilitychange'].forEach(function (guard) {
    ok(manual.indexOf(guard) >= 0, 'the masthead still honours ' + guard);
  });
  ok((manual.match(/give_up\(\)/g) || []).length >= 4,
    'and gives up rather than half-rendering when something is missing');
  ok(/\.masthead\.dead \{ display: none/.test(manual),
    'giving up actually hides the band');

  /* A sprite sheet that never resolves calls neither callback, so nothing
     below it ever runs and an empty 300px box sits above the title.  The
     watchdog covers that - but it has to ask whether the sheet *arrived*, not
     whether anything has been *drawn*: the band deliberately does not draw
     while scrolled out of view, and a paint-based watchdog retired a healthy
     masthead on any reload that restored the scroll position. */
  ok(/if \(!arrived\) give_up\(\);/.test(manual),
    'a sheet that never resolves retires the band');
  ok(/arrived = true;/.test(manual) && !/painted = true/.test(manual),
    'and the watchdog waits on the sheet, not on a first paint');

  /* The sheet is the game's own artwork, generated by a committed tool.  A
     hero drawn from hand-made art could drift away from the real ships
     without anything failing, which is the trap the screenshots already set
     once - so the generator stays in the repository next to its output. */
  ok(fs.existsSync(path.join(root, 'shots/hero-sprites.png')),
    'the masthead sprite sheet is committed');
  ok(fs.existsSync(path.join(root, 'tools/hero-sprites.html')),
    'and so is the tool that regenerates it from js/sprites.js');

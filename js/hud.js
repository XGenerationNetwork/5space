...
(function (SS) {
  ...
  /* Closing a menu is also the moment the on-screen controls come back.
   *
   * While an overlay is up, `#overlay:not(.hidden) ~ #touch` takes the whole
   * control layer out of the layout - so any measuring done in that window
   * sees zeroes, and layoutTouch bails rather than deciding from them.  That
   * leaves the layout owed, and here is where it is paid: the three places a
   * menu closes all come through this.
   *
   * The bug this fixes: the game boots with the title screen open, so the
   * first layout ran against a hidden layer, concluded the utility stack fit
   * nowhere, and withdrew it - and nothing recomputed until the phone was
   * rotated, which is exactly how it presented. */
  function hideOverlay() {
    overlay.classList.add('hidden');
    overlay.innerHTML = '';
    overlayOpen = false;
    if (hud.layoutTouch) relayout();
  }
  ...
  hud.layoutTouch = function () {
    ...
    /* Rendered, not merely un-hidden.
     *
     * A menu takes the layer out of the layout with `#overlay:not(.hidden) ~
     * #touch`, which sets no class here - so this used to run happily against
     * a layer that had no boxes at all.  Every rect came back as zeroes, which
     * reads as "every button is jammed into the top-left corner and fits
     * nowhere", and the utility stack was withdrawn on that basis.  The game
     * boots with the title screen up, so that was the *first* layout of every
     * session; only a rotation recomputed it.
     *
     * position:fixed rules out offsetParent as the test, hence client rects. */
    if (!touchLayer.getClientRects().length) return;
